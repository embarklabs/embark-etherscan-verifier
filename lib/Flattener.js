...
class Flattener {
  ...
  _doVerify(contract, flattenedCode, apiKey, callback) {
    const compilerVersion = 'get this from somewhere';
    const optimizationUsed = 'get from config';
    const optimizationRuns = 'get from config';
    ...
    this.events.request('deploy:contract:object', contract, (err, deployObject) => {
      ...
      console.log('Ok?', deployArguments);
      return callback();
  ...
  verify(apiKey, contractName, callback) {
    ...
    this.events.request("contracts:contract", contractName, (contract) => {
      ...
      async.waterfall([
        ...
        this._doVerify(contract, content, apiKey, callback);
      });
    });
  }
